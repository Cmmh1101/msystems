...
### Newsletter broadcasts (admin-composed)

Separate from the automated nurture sequence above — this is a manual, admin-composed email sent to a chosen list on demand, from `/admin/newsletter` (migration 0012 adds `clients.subscribed` and the `newsletters` history table).

**Lists**, resolved live from the database at send time (`lib/newsletter.ts`'s `resolveSegmentRecipients()`) rather than a separately-synced list, and every one always excludes `subscribed = false`:
- **All lists** — every client and every contact, de-duplicated by email
- **Clients** — from `clients`
- **Newsletter** — contacts with `source = 'newsletter'`, i.e. people who used the public signup box in the site footer (`components/NewsletterSignup.tsx` → `/api/newsletter`) — a dedicated intake path, distinct from the diagnostic quiz's own opt-in checkbox, so this list means exactly "signed up for the newsletter specifically"
- **Diagnostic** — contacts with `source = 'diagnostic'`

**Composing** uses a real rich-text editor (`components/admin/NewsletterEditor.tsx`, built on TipTap) — bold/italic/underline, bullet/numbered lists, left/center/right alignment, links, and inline images uploaded through the same `/api/admin/upload` route the blog editor uses (now accepting an optional `folder` field, so newsletter images land under `newsletters/` in the `blog-images` bucket instead of `posts/`). Images can carry their own link via a custom `LinkableImageExtension.ts` that adds an `href` attribute to TipTap's Image node and wraps it in an `<a>` on render — plain TipTap images have no way to be clickable themselves. One non-obvious TipTap detail: an attribute meant to be read back out in a node's own `renderHTML` (as opposed to written onto the HTML element as a real attribute) has to be read from `node.attrs`, not the `HTMLAttributes` argument — an attribute whose own `renderHTML` returns `{}` (to keep it off the actual DOM element) is for that exact reason stripped out of `HTMLAttributes` before the node ever sees it.

**Sending** goes out as real HTML email (`lib/newsletterEmail.ts`'s `wrapNewsletterHtml()` — a minimal self-contained shell, since email clients strip `<style>` tags unpredictably and have poor CSS support generally) with an auto-generated plain-text fallback (`htmlToPlainText()`) for clients that prefer it, both parts carrying a working unsubscribe link. This is the first HTML email anywhere in this codebase — every other transactional email (contact replies, diagnostic results, portal invites, the nurture nudge) stays plain text, so `/api/admin/newsletter/test` and `/send` are the only two routes using `html:` on `resend.emails.send()`. Composing supports rich formatting; every other email intentionally doesn't. **Send test** goes to any typed address without touching the send history; **Send to list** requires an explicit confirm step and records a `newsletters` row (subject, body, list, recipient count, timestamp) shown as history on the same page. `/api/unsubscribe` now updates `subscribed` on both `contacts` and `clients` by email, not just `contacts` as before.
...
`clients` and `projects` tables (migration 0008), both service_role-only for now — RLS is enabled on both but has no policies yet; client-role policies get added once client portal auth exists (spec step 3). `/admin/clients` lists clients with a live project count per row (`projects(count)` embedded select, not a separate query per row) plus three "needs attention" columns — **Requests** (tickets in `client_request`, awaiting review), **To Do**, and **Pending** (billing status `needs_quote` or `quoted`) — computed by fetching every ticket once with its `projects!inner(client_id)` embed and aggregating in-memory by client, rather than a separate filtered-count query per client row; `/admin/clients/[id]` handles inline editing of name/email/company/status plus adding/managing that client's projects.
